...
st.set_page_config(page_title="MLB 선수 검색기", page_icon="⚾", layout="centered")

st.title("⚾ MLB 선수 성적 검색기")
st.write("선수의 이름을 영문으로 입력하고 시즌을 선택하세요. (예: Ohtani, Judge, Trout)")

# 1. 입력 폼 (선수 이름 및 시즌 선택)
...
# 캐싱을 이용해 데이터 로딩 속도 최적화
@st.cache_data(ttl=3600)
def get_fan_graphs_data(target_season):
    """FanGraphs 기반의 batting_stats (wRC+ 포함) 데이터를 불러옵니다."""
    try:
        # qualifier 없이 해당 시즌 전체 선수 데이터 로드
...
if search_button and player_name.strip():
    with st.spinner("데이터를 불러오는 중입니다..."):
        # A. MLB Official StatsAPI를 통해 선수 ID 및 기본 사진/프로필 검색
        ...
        if not lookup_results:
            ...
            # MLB Headshot API를 활용한 공식 선수 사진 URL
            ...
            # B. MLB Official StatsAPI에서 기본 통계 (안타, 홈런, 삼진, 볼넷) 조회
            ...
            # C. pybaseball (FanGraphs 데이터)을 이용한 wRC+ 추출
            wrc_plus = "N/A"
            fg_df = get_fan_graphs_data(season)
            
            if fg_df is not None and not fg_df.empty:
                # 이름 매칭 (성 또는 이름으로 검색)
                matched_player = fg_df[fg_df['Name'].str.lower() == full_name.lower()]
                if matched_player.empty:
                    # 완벽히 일치하지 않을 경우 부분 매칭 시도
                    matched_player = fg_df[fg_df['Name'].str.contains(player_name, case=False, na=False)]
                
                if not matched_player.empty and 'wRC+' in matched_player.columns:
                    wrc_plus = matched_player.iloc[0]['wRC+']
                    # 소수점 정리
                    if isinstance(wrc_plus, (int, float)):
            ...
            with col_info:
                ...
                st.write(f"**조회 시즌:** {season}년")

            st.write("---")
            st.markdown("### 📊 시즌 타격 성적")
            ...
            with m_col3:
                st.metric(label="wRC+", value=f"{wrc_plus}")
